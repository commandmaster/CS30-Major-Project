...
export class Rigidbody{
    ...
    stepSim(dt){
        ...
        this.#calculateTransform();
        this.#colliders.forEach(collider => collider.stepSim((dt)));
    }
...
export class CollisionData{
    constructor(axis, overlap, collider1, collider2, collisionPoints,){
        ...
        this.collider1 = collider1; // First collider involved in the collision
        this.collider2 = collider2; // Second collider involved in the collision
    }
}  
...
export class PhysicsEngine{
    ...
    static ConvexCollider = ConvexCollider;
    static CircleCollider = CircleCollider;
    static RectangleCollider = RectangleCollider;
    static TriangleCollider = TriangleCollider;
    static Vec2 = Vec2;


    constructor(maxTimeStep){
        this.maxTimeStep = maxTimeStep; // Maximum time step for the physics engine in ms
        this.rigidBodies = [];
        this.currentlyCollidingPairs = [];
    }
    ...
    stepSim(dt){
        this.currentlyCollidingPairs = []; // Reset the currently colliding pairs
        dt = Math.min(dt, this.maxTimeStep / 1000); // Limit the time step to the maximum time step

        // dt is the time step in seconds
        for (const rigidBody of this.rigidBodies){
            rigidBody.stepSim(dt);
        }
                ...
                for (const collider1 of body1.colliders){
                    for (const collider2 of body2.colliders){
                        ...
                        if (collider1.type === 'circle' && collider2.type === 'convex'){
                            ...
                            if (isNaN(collider1.position.x) || isNaN(collider1.position.y) || isNaN(collider2.position.x) || isNaN(collider2.position.y)){
                                console.log('NaN detected');
                                throw new Error('NaN detected on loop #' + String(loop));
                            }
                            ...
                            const collisionData = SAT.circleToPoly(collider2, collider1);
                            ...
                            if (collisionData){
                                CollisionSolver.resolveCollision(body2, body1, collisionData);
...
const engineLoader = (maxTimeStep) => {
    const physicsEngine = new PhysicsEngine(maxTimeStep);
    return {engine: physicsEngine, class: physicsEngine.constructor};
}
